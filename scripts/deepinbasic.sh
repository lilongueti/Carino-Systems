#!/bin/bash
echo $'\e[1;32m'Basic profile for Deepin$'\e[0m'
echo $'\e[1;32m'Perfil básico para Deepin$'\e[0m'
echo $'\e[1;32m'Базовый профиль для Deepin$'\e[0m'
echo $'\e[1;32m'Deepin の基本プロファイル$'\e[0m'
echo $'\e[1;32m'--------------------------------------$'\e[0m'
#Getting info
...
then
    ...
    sudo dnf install https://github.com/rustdesk/rustdesk/releases/download/1.1.8/rustdesk-1.1.8-Deepin28-centos8.rpm -y
fi
...
if [ $choice == y ]
then
    sudo dnf install steam goverlay lutris mumble -y && flatpak install flathub com.discordapp.Discord
