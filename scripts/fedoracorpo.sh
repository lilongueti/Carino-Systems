...
echo $'\e[1;32m'Work Setup for Fedora$'\e[0m'
sudo rpm --import https://packages.microsoft.com/keys/microsoft.asc && sudo dnf config-manager --add-repo https://packages.microsoft.com/yumrepos/edge && sudo dnf config-manager --add-repo https://packages.microsoft.com/yumrepos/vscode && sudo dnf update -y && sudo dnf install https://download1.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://download1.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm -y && sudo dnf install mpv google-chrome-stable microsoft-edge-stable code keepassxc telegram-desktop thunderbird transmission gimp htop neofetch mediainfo obs-studio wine NetworkManager-tui yt-dlp cmake gnome-tweaks gnome-extensions-app elinks git xkill -y && flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo && flatpak install flathub com.spotify.Client com.microsoft.Teams com.skype.Client us.zoom.Zoom org.onlyoffice.desktopeditors com.anydesk.Anydesk -y
