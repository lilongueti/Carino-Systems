...
case $choice in
  ...
  "1")
    ...
    sudo apt install aptitude mpv google-chrome-stable keepassxc telegram-desktop thunderbird transmission gimp htop neofetch mediainfo obs-studio wine cmake gnome-tweaks elinks git -y && flatpak install flathub com.spotify.Client us.zoom.Zoom org.onlyoffice.desktopeditors com.anydesk.Anydesk -y
    support=y
    ;;
  ...
  "3")
    ...
    sudo apt update -y && sudo apt upgrade -y
    #Installing packages
    sudo apt install aptitude apt-transport-https celluloid keepassxc thunderbird transmission gimp htop powertop remmina neofetch mediainfo obs-studio barrier wine network-manager yt-dlp cmake lshw gnome-tweaks gnome-extensions-app elinks git tldr qt5-qtbase-devel python3-vapoursynth bridge-utils cifs-utils speedtest-cli libvirt libvirt-devel virt-install qemu-kvm qemu qemu-img python3 python3-pip virt-manager tigervnc-server cockpit xrdp powershell dnf-plugins-core dhcp-server -y && flatpak install flathub org.telegram.desktop us.zoom.Zoom com.dropbox.Client com.spotify.Client org.onlyoffice.desktopeditors com.anydesk.Anydesk com.skype.Client com.microsoft.Teams com.visualstudio.code com.microsoft.edge com.github.gi_lom.dialect com.slack.Slack com.usebottles.bottles -y
    support=y
    sharedfolder=y
    ;;
  #Exit program
  ...
  "0")
    ...
    sudo apt update -y && sudo apt upgrade -y
    #Installing packages
    sudo apt install aptitude keepassxc librewolf thunderbird transmission gimp krita kdenlive shotcut blender htop powertop remmina neofetch mediainfo obs-studio barrier wine network-manager yt-dlp cmake lshw elinks git mumble goverlay tldr steam qt5-qtbase-devel python3-vapoursynth bridge-utils cifs-utils speedtest-cli libvirt libvirt-devel virt-install qemu-kvm qemu qemu-img python3 python3-pip virt-manager tigervnc-server cockpit xrdp powershell dnf-plugins-core dhcp-server -y && flatpak install flathub com.visualstudio.code com.microsoft.edge org.telegram.desktop com.spotify.Client org.onlyoffice.desktopeditors com.anydesk.Anydesk com.github.gi_lom.dialect com.skype.Client com.usebottles.bottles -y
...
then
    ...
    then
        ...
        wget -q https://github.com/rustdesk/rustdesk/releases/download/1.1.9/rustdesk-1.1.9.deb -y && sudo dpkg -i rustdesk-1.1.9.deb && rm rustdesk-1.1.9.deb
    fi
fi
#MPV
#echo $mpv
...
then
    ...
    then
      ...
      sudo aptitude install mpv -y && sudo git clone https://github.com/mpv-player/mpv && cd mpv/ && sudo ./bootstrap.py && sudo ./waf configure --enable-vapoursynth && sudo ./waf && sudo ./waf install && cd .. && sudo rm -r mpv
    fi
#else
