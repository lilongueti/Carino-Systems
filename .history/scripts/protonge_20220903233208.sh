...
then
    ...
    if [ $? -eq 0 ]
        then
            ...
            echo "${GREEN}ProtonEG $PROTONVERSION has been installed.${ENDCOLOR}"
        else
            PROTONVERSION=$(($PROTONVERSION-1))
            echo "${GREEN}You already have the latest version ($PROTONVERSION).${ENDCOLOR}"
    fi
else
    sudo mkdir ~/.steam/
    ...
    echo "${GREEN}ProtonEG $PROTONVERSION has been installed by creating folders.${ENDCOLOR}"
fi